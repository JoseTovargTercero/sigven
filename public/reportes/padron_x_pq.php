...
$primeraHoja = 1;
$query = "SELECT name2 FROM continente WHERE id='$continente_id'";
...
$sql4 = "SELECT * FROM ciudad WHERE pais_id= ?";
$comunidades = $base->prepare($sql4);

$sql4 = "SELECT * FROM pais WHERE continente_id='$continente_id'";
$ubchs = $base->prepare($sql4);
...
$ubchs->execute(array());
while ($idRowUbch = $ubchs->fetch(PDO::FETCH_ASSOC)) {

    $nombreUbch = $idRowUbch['name1'];
    $idUbch = $idRowUbch['id'];


$comunidades->execute(array($idUbch));
...
->setCellValue('A4', $idRow['name'])
->setCellValue('A6', $nombreUbch.' - '.$nombrePq);
...
header('Content-Disposition: attachment;filename="Padron - '.$nombrePq.' - SIGVEN.xlsx"');
