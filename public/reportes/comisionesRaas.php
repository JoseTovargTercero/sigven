...
$comision = $_POST['comisionesRaasInput'];
...
header( 'Content-Disposition: attachment; filename=RAAS - '.$comision.' - SIGVEN - '.$nombreMunicipio.' - '.date('Y_m_d h-i a').' .xls' );
?>
<head>
   <meta charset="utf-8">
</head>
...
<p class='titulo'><?php echo ucwords(mb_strtolower($comision)); ?> - Estructura RAAS de <?php echo ucwords(mb_strtolower($nombreMunicipio)); ?><br><br></p>
...
$carnetizado = $base->prepare($sql);
    ...
    $query22="SELECT ciudad.name, pais.name1, continente.name2, estructuraraas.nombre, estructuraraas.cedula, estructuraraas.telefono, estructuraraas.cv
    FROM estructuraraas
    ...
    WHERE estructuraraas.cargo='$comision' AND estructuraraas.municipio='$municipio_id' ORDER BY `parroquia`,`ubch`,`comunidad`";  
