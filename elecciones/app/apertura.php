...
if (isset($_POST["cedula"]) && isset($_POST["cantidad"])) {
    ...
    if ($result) {
      ...
      if ($tipo_user != 'inst') {
          ...
          $stmt = mysqli_prepare($conexion_app, "SELECT * FROM `tablamesa` WHERE id = ? AND HORA_APERTURA=''");
          ...
          $result = $stmt->get_result();
            if (!$result->num_rows > 0) {
              ...
              $stmt2 = $conexion_app->prepare("UPDATE `tablamesa` SET `HORA_APERTURA` = ?, `ELECTORES_APERTURA` = ?, `USER_REPORTE` = ? WHERE id = ?");
